 ...
 *
 */
public abstract class CommonPageMethods {
	 ...
	 * @param report
	 */
	protected CommonPageMethods(WebDriver browser) {
	...
	protected boolean isElementPresent(WebElement element){
		try{
			...
			if(element.isDisplayed()){
				//Common.testStepPassed("Element is present in the application");
				return true;
			}else{
				//Common.testStepFailed("Element is not present in the application");
				return false;
			}
		}catch(Exception ex){
			return false;		
